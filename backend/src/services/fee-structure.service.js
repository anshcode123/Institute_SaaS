...
const { FEE_TYPE, COURSE_PAYMENT_MODE } = require('../constants/roles');
...
async function createFeeStructure(instituteId, actorUserId, data) {
  ...
  // Belt-and-suspenders: the validator already checks this with float
  // math at the request boundary; re-verify with Decimal here since this
  // is the authoritative check before anything is persisted.
  ...
  if (rest.feeType === FEE_TYPE.COURSE && !rest.coursePaymentMode) {
    throw new ValidationError('coursePaymentMode is required for COURSE fees');
  }
  if (rest.feeType === FEE_TYPE.COURSE
    && rest.coursePaymentMode === COURSE_PAYMENT_MODE.FULL
    && installments.length !== 1) {
    throw new ValidationError('COURSE FULL fees require exactly one installment');
  }
  if (rest.feeType === FEE_TYPE.COURSE
    && rest.coursePaymentMode === COURSE_PAYMENT_MODE.EMI
  ...
  return prisma.$transaction(async (tx) => {
    const structure = await tx.feeStructure.create({
      data: { ...rest, instituteId, batchId: batchId ?? null, createdById: actorUserId },
    });
