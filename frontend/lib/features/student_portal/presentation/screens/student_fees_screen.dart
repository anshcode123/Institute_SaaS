...
/// Read-only fee view - no record/edit affordances exist anywhere on
/// this screen since a STUDENT token can never call the mutating
/// /payments or /fees endpoints (Institute-Admin-only, enforced
/// server-side regardless of what this screen shows or hides).
class StudentFeesScreen extends ConsumerWidget {
  const StudentFeesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncFees = ref.watch(_myFeesProvider);
    return Scaffold(
      ...
      body: asyncFees.when(
        ...
        error: (err, _) => ErrorState(
            ...
            onRetry: () => ref.invalidate(_myFeesProvider)),
...
final _myFeesProvider =
    FutureProvider.autoDispose<StudentFeesResult>((ref) async {
  return ref.watch(studentPortalRepositoryProvider).getMyFees();
});
