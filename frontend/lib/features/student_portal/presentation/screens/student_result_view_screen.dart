...
/// Same visual layout as the Institute Admin/Teacher result detail
/// screen, wired to the self-scoped /student/results/:id endpoint
/// instead - reuses the StudentResultDetail model, not the data source.
class StudentResultViewScreen extends ConsumerWidget {
  const StudentResultViewScreen({super.key, required this.resultId});

  final String resultId;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncDetail = ref.watch(_resultProvider(resultId));
    ...
    return Scaffold(
      ...
      body: asyncDetail.when(
        ...
        error: (err, _) => ErrorState(
            ...
            onRetry: () => ref.invalidate(_resultProvider(resultId))),
...
final _resultProvider =
    FutureProvider.family<StudentResultDetail, String>((ref, resultId) async {
  return ref.watch(studentPortalRepositoryProvider).getMyResultDetail(resultId);
});
