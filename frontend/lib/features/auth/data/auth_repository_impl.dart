...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<AuthSession> loginSuperAdmin({required String email, required String password}) async {
    final response = await _post('/auth/super-admin/login', {'email': email, 'password': password});
  ...
  @override
  Future<AuthSession> loginTeacher({required String email, required String password}) async {
    final response = await _post('/auth/teacher/login', {'email': email, 'password': password});
    return _persistSession(response);
  }

  @override
  Future<AuthSession?> restoreSession() async {
    final accessToken = await SecureStorage.instance.read(AuthStorageKeys.accessToken);
    final refreshToken = await SecureStorage.instance.read(AuthStorageKeys.refreshToken);
    final userJson = await SecureStorage.instance.read(AuthStorageKeys.userJson);
  ...
  Future<AuthSession> _persistSession(Map<String, dynamic> data) async {
    ...
    await SecureStorage.instance.write(AuthStorageKeys.accessToken, accessToken);
    await SecureStorage.instance.write(AuthStorageKeys.refreshToken, refreshToken);
